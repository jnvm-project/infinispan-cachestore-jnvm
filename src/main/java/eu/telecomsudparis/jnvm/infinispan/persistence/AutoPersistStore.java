...
import eu.telecomsudparis.jnvm.infinispan.persistence.configuration.AutoPersistStoreConfiguration;
...
@Store
@MetaInfServices
@ConfiguredBy(AutoPersistStoreConfiguration.class)
public class AutoPersistStore<I extends OffHeapObject, J extends OffHeapObject, K extends Convertible<I>, V extends Convertible<J>> implements AdvancedLoadWriteStore<K, V> {

    private AutoPersistMap<I, J, K, V> backend = null;

    private InitializationContext ctx;
    private MarshalledEntryFactory marshalledEntryFactory;
    private AutoPersistStoreConfiguration configuration;
    ...
    public void start() {
        if (backend == null) {
            backend = AutoPersistMap.recover( ctx.getCache().getName(), 40000000 );
        }
    }
    ...
    public void write(MarshalledEntry<? extends K, ? extends V> marshalledEntry) {
        backend.putConvert(marshalledEntry.getKey(), marshalledEntry.getValue());
    }
